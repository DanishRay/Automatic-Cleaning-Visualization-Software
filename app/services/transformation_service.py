...
class TransformationService:
    ...
    def apply_transformations(
        filename: str, operations: list = None, r_script: str = None
    ) -> pd.DataFrame:
        ...
        if r_script and r_script.strip():
            with tempfile.TemporaryDirectory() as temp_dir:
                in_csv = (
                    os.path.abspath(os.path.join(temp_dir, "input.csv"))
                    .replace("\\", "/")
                )
                out_csv = (
                    os.path.abspath(os.path.join(temp_dir, "output.csv"))
                    .replace("\\", "/")
                )
                ...
                if result.returncode != 0:
                    raise RuntimeError(
                        f"R execution error: {result.stderr.strip()}"
                    )
                ...
                if os.path.exists(out_csv):
                    ...
                    raise RuntimeError(
                        "R script failed to produce output dataset."
                    )
            return df
